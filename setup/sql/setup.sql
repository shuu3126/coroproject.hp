-- ============================================================
-- CORO PROJECT セットアップSQL
-- 使い方:
--   ローカル(XAMPP): phpMyAdminでそのままインポート
--   本番サーバー:    mysql -u root -p < setup.sql
--   共有サーバー:    管理画面でDBを作成・選択してから admin/install.sql をインポート
-- ============================================================

-- データベース作成
CREATE DATABASE IF NOT EXISTS db_coroproject_1
  CHARACTER SET utf8mb4
  COLLATE utf8mb4_unicode_ci;

-- 本番用ユーザー作成・権限付与
-- ※ローカルXAMPPではrootで接続するためこのユーザーは不要ですが、
--   本番サーバーでは必須です。パスワードは環境に合わせて変更してください。
CREATE USER IF NOT EXISTS 'db_coroproject'@'localhost' IDENTIFIED BY 'FwMMCTUO';
GRANT ALL PRIVILEGES ON db_coroproject_1.* TO 'db_coroproject'@'localhost';
FLUSH PRIVILEGES;

USE db_coroproject_1;
...
-- ============================================================
-- 既存DB補正: 現行コードで必要なカラム・インデックスを追加
--   ※ MySQL / MariaDB の差で止まりやすい条件付きADD COLUMN構文は使わない
-- ============================================================

SET @ddl = (
  SELECT IF(COUNT(*) > 0,
    'ALTER TABLE `accounting_invoices` MODIFY COLUMN `talent_id` VARCHAR(191) NULL',
    'SELECT 1'
  )
...
SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `accounting_invoices` ADD COLUMN `client_id` VARCHAR(191) NULL AFTER `talent_id`',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
...
SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `accounting_invoices` ADD COLUMN `division` VARCHAR(20) NOT NULL DEFAULT ''production'' AFTER `note`',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
...
SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `accounting_invoices` ADD COLUMN `deal_id` VARCHAR(191) NULL AFTER `division`',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
...
SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `accounting_invoices` ADD COLUMN `project_id` VARCHAR(191) NULL AFTER `deal_id`',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
...
SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `accounting_invoices` ADD INDEX `idx_accounting_invoices_client` (`client_id`)',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'accounting_invoices'
    AND INDEX_NAME = 'idx_accounting_invoices_client'
);
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `accounting_invoices` ADD INDEX `idx_accounting_invoices_division` (`division`)',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'accounting_invoices'
    AND INDEX_NAME = 'idx_accounting_invoices_division'
);
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `accounting_invoices` ADD INDEX `idx_accounting_invoices_deal` (`deal_id`)',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'accounting_invoices'
    AND INDEX_NAME = 'idx_accounting_invoices_deal'
);
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `accounting_invoices` ADD INDEX `idx_accounting_invoices_project` (`project_id`)',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'accounting_invoices'
    AND INDEX_NAME = 'idx_accounting_invoices_project'
);
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `inquiries` ADD COLUMN `status` VARCHAR(20) NOT NULL DEFAULT ''read'' AFTER `message`',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
...
SET @ddl = (
  SELECT IF(COUNT(*) > 0,
    'ALTER TABLE `inquiries` MODIFY COLUMN `status` VARCHAR(20) NOT NULL DEFAULT ''unread''',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
...
SET @ddl = (
  SELECT IF(COUNT(*) = 0,
    'ALTER TABLE `inquiries` ADD INDEX `idx_inquiries_status` (`status`)',
    'SELECT 1'
  )
  FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE()
