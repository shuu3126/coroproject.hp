...
foreach ($talents as $talent) {
    ...
    $fx = (float)load_app_settings($pdo, $config)['fx_default_rate'];
...
$recentLogs = $pdo->query("SELECT l.created_at, l.summary, COALESCE(u.display_name, 'system') AS user_name FROM admin_logs l LEFT JOIN admin_users u ON u.id = l.user_id ORDER BY l.created_at DESC LIMIT 10")->fetchAll();

require __DIR__ . '/_header.php';
?>
<main class="page-container">
  <section class="card-grid two">
    <a class="card stat-card" href="<?= h($baseUrl) ?>/accounting/revenues.php"><div class="muted">未提出収益</div><div class="stat-number"><?= h((string)$unsubmitted) ?></div><p>今月まだ収益が登録されていないタレント</p></a>
    <a class="card stat-card" href="<?= h($baseUrl) ?>/accounting/invoices.php"><div class="muted">請求可能</div><div class="stat-number"><?= h((string)$invoiceReady) ?></div><p>回収ライン5,000円以上の対象</p></a>
