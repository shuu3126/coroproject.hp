...
<label><span>タレント（任意）</span><select name="talent_id"><option value="">選択しない</option><?php foreach($talents as $t): ?><option value="<?= h((string)$t['id']) ?>" <?= selected($row['talent_id'],$t['id']) ?>><?= h($t['display_name']) ?></option><?php endforeach; ?></select></label>
