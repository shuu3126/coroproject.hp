<?php
function accounting_settings( $pdo) {
...
function accounting_get_uninvoiced_months( $pdo, $talentId, $year, $month) {
...
function accounting_calc_office_share_jpy_for_month( $pdo, $talentId, $year, $month, $fxRate) {
    ...
    $totalJpy = 0.0;
    foreach ($rows as $row) {
        $sum = (float)((isset($row['total']) ? $row['total'] : 0));
        $currency = strtoupper((string)((isset($row['currency']) ? $row['currency'] : 'JPY')));
...
function accounting_insert_journal_for_invoice( $pdo, $invoiceId, $talentId, $invoiceNo, $year, $month, $amount, $note = '') {
...
function accounting_generate_document_html( $data, $type, $absoluteDir, $relativePrefix) {
    ...
    $period = (isset($data['period_label']) ? $data['period_label'] : sprintf('%d年%02d月', $data['year'], $data['month']));
