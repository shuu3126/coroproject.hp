...
$errors = [];
$input  = ['name' => '', 'address' => '', 'work_type' => '', 'start_date' => '', 'end_date' => '', 'supervisor_id' => '', 'status' => '準備中', 'memo' => ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("
            INSERT INTO sites (name, address, work_type, start_date, end_date, supervisor_id, status, memo)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            $input['name'],
        ...
        log_action($pdo, 'create', '現場', $input['name'], $input['work_type'] ?? '');
...
<main class="px-4 py-4 md:px-8 md:py-6 max-w-2xl mx-auto">
  ...
  <form method="post" class="bg-white rounded-xl shadow-sm p-5 space-y-4">
    ...
    <div class="flex gap-3 pt-2">
      <a href="/tamiya-home/pages/sites/index.php"
         class="flex-1 text-center bg-gray-100 text-gray-600 font-bold py-3 rounded-xl text-sm">キャンセル</a>
      <button type="submit"
        class="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-xl text-sm">登録する</button>
    </div>
