<?php
require_once __DIR__ . '/../db.php';
require_once __DIR__ . '/_asset.php';

function esc($s) {
    return htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8');
}

// ---------- ID 蜿門ｾ・----------
$id = $_GET['id'] ?? '';
if ($id === '') {
    http_response_code(404);
    echo 'ID not found';
    exit;
}

// ---------- 繧ｿ繝ｬ繝ｳ繝・莉ｶ ----------
$sql = "SELECT * FROM talents WHERE id = :id LIMIT 1";
$stmt = $pdo->prepare($sql);
$stmt->execute([':id' => $id]);
$talent = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$talent) {
    http_response_code(404);
    echo 'Talent not found';
    exit;
}

// ---------- SNS繝ｪ繝ｳ繧ｯ ----------
$stmt = $pdo->prepare("
    SELECT *
    FROM talent_links
    WHERE talent_id = :id
    ORDER BY id ASC
");
$stmt->execute([':id' => $id]);
$links = $stmt->fetchAll(PDO::FETCH_ASSOC);

// ---------- 驟堺ｿ｡繝励Λ繝・ヨ繝輔か繝ｼ繝 ----------
$stmt = $pdo->prepare("
    SELECT *
    FROM talent_platforms
    WHERE talent_id = :id
    ORDER BY id ASC
");
$stmt->execute([':id' => $id]);
$platforms = $stmt->fetchAll(PDO::FETCH_ASSOC);

// ---------- Avatar 繝代せ隱ｿ謨ｴ・・html 縺九ｉ縺ｮ逶ｸ蟇ｾ繝代せ縺ｫ縺吶ｋ・・----------
$avatar = $talent['avatar'] ?? '';
if ($avatar !== '') {
    if (strpos($avatar, '../') === 0) {
        // ../images/・・縺ｯ縺昴・縺ｾ縺ｾ菴ｿ縺医ｋ
    } elseif (strpos($avatar, './') === 0) {
        $avatar = '../' . ltrim(substr($avatar, 2), '/');
    } elseif (strpos($avatar, 'images/') === 0) {
        $avatar = '../' . $avatar;
    }
    $avatar = public_html_asset_url($talent['avatar'] ?? '');
}

// ---------- 繧ｿ繧ｰ・・ags_json・・----------
$tagChips = [];
if (!empty($talent['tags_json'])) {
    $tmp = json_decode($talent['tags_json'], true);
    if (is_array($tmp)) {
        // ["髮題ｫ・,"豁・] 縺ｧ繧・{"main":["髮題ｫ・,"豁・]} 縺ｧ繧ゅ◎縺薙◎縺捺鏡縺医ｋ繧医≧縺ｫ
        if (array_keys($tmp) === range(0, count($tmp) - 1)) {
            $tagChips = $tmp;
        } else {
            foreach ($tmp as $v) {
                if (is_array($v)) {
                    $tagChips = array_merge($tagChips, $v);
                } else {
                    $tagChips[] = $v;
                }
            }
        }
    }
}

// ---------- 隧ｳ邏ｰ繝励Ο繝輔ぅ繝ｼ繝ｫ・・ong_bio_json・・----------
$profileParagraphs = [];
if (!empty($talent['long_bio_json'])) {
    $tmp = json_decode($talent['long_bio_json'], true);
    if (is_array($tmp)) {
        // ["譁・","譁・"] 縺ｾ縺溘・ {"body":["譁・","譁・"]} 繧呈Φ螳・
        if (isset($tmp['body']) && is_array($tmp['body'])) {
            $profileParagraphs = $tmp['body'];
        } elseif (array_keys($tmp) === range(0, count($tmp) - 1)) {
            $profileParagraphs = $tmp;
        }
    }
}
if (!$profileParagraphs && !empty($talent['bio'])) {
    // 縺ｪ縺代ｌ縺ｰ bio 繧・縲・陦後↓蛻・￠縺ｦ蜃ｺ縺・
    $profileParagraphs = preg_split('/\r\n|\r|\n/', $talent['bio']);
}

// ---------- 讎りｦ∫畑縺ｮ霑ｽ蜉諠・ｱ・・ong_bio_json 縺ｫ縺ゅｌ縺ｰ菴ｿ縺・ｼ・----------
$extra = [
    'birthday'   => '',
    'height'     => '',
    'color'      => '',
    'fanmark'    => '',
    'hashtags'   => [],  // general / live / art / fan 縺ｪ縺ｩ繧呈Φ螳・
];

if (!empty($talent['long_bio_json'])) {
    $tmp = json_decode($talent['long_bio_json'], true);
    if (is_array($tmp)) {
        foreach ($extra as $key => $_) {
            if (isset($tmp[$key])) {
                $extra[$key] = $tmp[$key];
            }
        }
    }
}
?>
<!doctype html>
<html lang="ja">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1" />
  <title><?= esc($talent['name']) ?> | Talents | CORO PROJECT</title>
  <meta name="description" content="<?= esc($talent['bio'] ?: $talent['name'].' 縺ｮ繝励Ο繝輔ぅ繝ｼ繝ｫ繝壹・繧ｸ縺ｧ縺吶・) ?>">
  <link rel="canonical" href="https://coroproject.jp/production/html/talent.php?id=<?= esc($talent['id']) ?>">

  <!-- OGP / Twitter -->
  <meta property="og:site_name" content="CORO PROJECT">
  <meta property="og:type" content="website">
  <meta property="og:title" content="<?= esc($talent['name']) ?> | CORO PROJECT">
  <meta property="og:description" content="<?= esc($talent['bio'] ?: $talent['name'].' 縺ｮ繝励Ο繝輔ぅ繝ｼ繝ｫ繝壹・繧ｸ縺ｧ縺吶・) ?>">
  <meta property="og:url" content="https://coroproject.jp/production/html/talent.php?id=<?= esc($talent['id']) ?>">
  <meta property="og:image" content="https://coroproject.jp/production/images/ogp.png">
  <meta name="twitter:card" content="summary_large_image">

  <!-- CSS 縺ｯ styles.css 縺縺代〒OK・・op.css 縺ｯ荳崎ｦ・ｼ・-->
  <link rel="stylesheet" href="../css/styles.css">
  <link rel="icon" type="image/png" href="../images/logo.png">
  <link rel="apple-touch-icon" href="../images/logo.png">
</head>
<body class="talent-page">
<div id="app" class="app visible">

  <!-- ===== Header ===== -->
  <header class="site-header">
    <div class="container header-inner">
      <a class="brand" href="../index.php">
        <img src="../images/toukalogo.png" alt="CORO PROJECT 繝ｭ繧ｴ" class="brand-logo">
        <span class="brand-text">CORO PROJECT</span>
      </a>

      <button class="nav-toggle" id="navToggle" aria-expanded="false" aria-controls="siteNav" aria-label="繝｡繝九Η繝ｼ繧帝幕縺・>
        <span></span><span></span><span></span>
      </button>

      <nav class="nav" id="siteNav" aria-label="繝｡繧､繝ｳ繝翫ン繧ｲ繝ｼ繧ｷ繝ｧ繝ｳ">
        <a href="../index.php#about">About</a>
        <a href="./news.php">News</a>
        <a href="./talents.php" aria-current="page">Talents</a>
        <a href="./audition.html">Audition</a>
        <a href="./contact.html">Contact</a>
      </nav>
    </div>
  </header>

  <main id="top">

    <!-- ===== Talent Hero・井ｸ企Κ・・===== -->
    <section class="talent-hero">
      <div class="talent-hero-bg" aria-hidden="true"></div>

      <div class="container talent-hero-inner">
        <div class="talent-hero-main">
          <p class="eyebrow">TALENT PROFILE</p>
          <h1 class="talent-name-main"><?= esc($talent['name']) ?></h1>
          <?php if (!empty($talent['kana'])): ?>
            <p class="talent-name-kana"><?= esc($talent['kana']) ?></p>
          <?php endif; ?>

          <?php if ($tagChips): ?>
            <div class="talent-tags">
              <?php foreach ($tagChips as $tag): ?>
                <span><?= esc($tag) ?></span>
              <?php endforeach; ?>
            </div>
          <?php endif; ?>

          <?php if (!empty($talent['bio'])): ?>
            <p class="talent-catch"><?= esc($talent['bio']) ?></p>
          <?php endif; ?>
        </div>

        <?php if ($avatar): ?>
          <!-- 笘・縺薙％縺・16:9 縺ｮ讓ｪ髟ｷ繝舌リ繝ｼ -->
          <div class="talent-hero-banner"
               style="background-image:url('<?= esc($avatar) ?>');"></div>
        <?php endif; ?>
      </div>
    </section>

    <!-- ===== Profile + 繧ｵ繧､繝画ュ蝣ｱ ===== -->
    <section class="section talent-profile-section">
      <div class="container talent-profile-inner">
        <div class="talent-profile-main">
          <h2 class="section-title">Profile</h2>

          <?php if ($profileParagraphs): ?>
            <?php foreach ($profileParagraphs as $p): ?>
              <?php if (trim($p) === '') continue; ?>
              <p><?= esc($p) ?></p>
            <?php endforeach; ?>
          <?php else: ?>
            <p>貅門ｙ荳ｭ縺ｧ縺吶・/p>
          <?php endif; ?>
        </div>

        <aside class="talent-profile-side">
          <!-- Overview -->
          <section class="talent-card">
            <h3 class="talent-card-title">Overview</h3>
            <dl class="talent-meta-list">
              <div>
                <dt>Status</dt>
                <dd><?= esc($talent['status'] ?: '窶・) ?></dd>
              </div>
              <div>
                <dt>Group</dt>
                <dd><?= esc($talent['talent_group'] ?: '-') ?></dd>
              </div>
              <div>
                <dt>Debut</dt>
                <dd><?= esc($talent['debut'] ?: '-') ?></dd>
              </div>
              <div>
                <dt>Last Active</dt>
                <dd><?= esc($talent['last_active'] ?: '-') ?></dd>
              </div>
              <?php if ($extra['birthday']): ?>
              <div>
                <dt>Birthday</dt>
                <dd><?= esc($extra['birthday']) ?></dd>
              </div>
              <?php endif; ?>
              <?php if ($extra['height']): ?>
              <div>
                <dt>Height</dt>
                <dd><?= esc($extra['height']) ?></dd>
              </div>
              <?php endif; ?>
              <?php if ($extra['color']): ?>
              <div>
                <dt>Color</dt>
                <dd><?= esc($extra['color']) ?></dd>
              </div>
              <?php endif; ?>
              <?php if ($extra['fanmark']): ?>
              <div>
                <dt>Fan mark</dt>
                <dd><?= esc($extra['fanmark']) ?></dd>
              </div>
              <?php endif; ?>
            </dl>
          </section>

          <!-- Streaming -->
          <section class="talent-card">
            <h3 class="talent-card-title">Streaming</h3>
            <?php if ($platforms): ?>
              <ul class="talent-link-list">
                <?php foreach ($platforms as $p): ?>
                  <li>
                    <a href="<?= esc($p['url']) ?>" target="_blank" rel="noopener">
                      <?= esc($p['name']) ?>
                    </a>
                  </li>
                <?php endforeach; ?>
              </ul>
            <?php else: ?>
              <p class="talent-card-note">貅門ｙ荳ｭ縺ｧ縺吶・/p>
            <?php endif; ?>
          </section>

          <!-- Links -->
          <section class="talent-card">
            <h3 class="talent-card-title">Links</h3>
            <?php if ($links): ?>
              <ul class="talent-link-list">
                <?php foreach ($links as $ln): ?>
                  <li>
                    <a href="<?= esc($ln['url']) ?>" target="_blank" rel="noopener">
                      <?= esc($ln['label']) ?>
                    </a>
                  </li>
                <?php endforeach; ?>
              </ul>
            <?php else: ?>
              <p class="talent-card-note">貅門ｙ荳ｭ縺ｧ縺吶・/p>
            <?php endif; ?>
          </section>
        </aside>
      </div>
    </section>

  </main>

    <!-- ===== Footer・亥・騾夲ｼ・===== -->
    <footer class="site-footer">
      <div class="container footer-inner">
        <div class="footer-col">
          <div class="brand brand--footer">
            <img src="../images/logo.png" alt="CORO PROJECT" class="footer-logo">
            <span class="brand-text">CORO PROJECT</span>
          </div>
          <p class="footer-desc">CORO PROJECT縺ｯVTuber縺ｮ繝励Ο繝・Η繝ｼ繧ｹ繝ｻ驟堺ｿ｡謾ｯ謠ｴ繝ｻ繧ｯ繝ｪ繧ｨ繧､繝・ぅ繝門宛菴懊ｒ陦後≧繝励Ο繝繧ｯ繧ｷ繝ｧ繝ｳ縺ｧ縺吶・/p>
          <div class="footer-actions">
            <a class="btn btn-primary" href="./contact.html">蝠上＞蜷医ｏ縺・/a>
          </div>
        </div>

        <div class="footer-col">
          <h4>Links</h4>
          <ul class="footer-links">
            <li><a href="./news.php">News</a></li>
            <li><a href="./talents.php">Talents</a></li>
            <li><a href="./audition.html">Audition</a></li>
            <li><a href="./privacy.html">Privacy Policy</a></li>
          </ul>
        </div>

        <div class="footer-col">
          <h4>Social</h4>
          <ul class="footer-links">
            <li><a href="#">YouTube</a></li>
            <li><a href="https://x.com/CoroProjectJP">X</a></li>
            <li><a href="#">Twitch</a></li>
          </ul>
        </div>
      </div>
      <div class="container footer-copy">
        <small>ﾂｩ <span id="year"></span> CORO PROJECT</small>
      </div>
    </footer>
  </div>

<script>
  document.getElementById('year').textContent = new Date().getFullYear();

  (function(){
    const btn = document.getElementById('navToggle');
    const nav = document.getElementById('siteNav');
    if (!btn || !nav) return;

    btn.addEventListener('click', () => {
      const open = btn.getAttribute('aria-expanded') === 'true';
      btn.setAttribute('aria-expanded', String(!open));
      document.body.classList.toggle('nav-open', !open);
    });
  })();
</script>
</body>
</html>
