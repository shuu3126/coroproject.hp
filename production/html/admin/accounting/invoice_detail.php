...
<div class="card mt-24"><h3>請求書</h3><div class="actions-inline"><?php if(!empty($invoice['invoice_pdf_path'])): ?><a class="ghost-btn" target="_blank" href="/<?= h($invoice['invoice_pdf_path']) ?>">請求書を開く</a><?php endif; ?><form method="post"><input type="hidden" name="action" value="regenerate_invoice_pdf"><button class="primary-btn" type="submit">請求書PDFを再出力する</button></form></div></div>
...
<div class="card mt-24"><h3>領収書</h3><div class="actions-inline"><?php if(!empty($invoice['receipt']) && !empty($invoice['receipt']['receipt_pdf_path'])): ?><a class="ghost-btn" target="_blank" href="/<?= h($invoice['receipt']['receipt_pdf_path']) ?>">領収書を開く</a><?php endif; ?><form method="post" data-confirm="領収書を発行または再発行しますか？"><input type="hidden" name="action" value="issue_receipt"><button class="primary-btn" type="submit">領収書を発行する</button></form></div></div>
