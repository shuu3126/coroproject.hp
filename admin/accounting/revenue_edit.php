...
<div class="form-grid two"><label><span>タレント</span><select name="talent_id" required><?php foreach($talents as $t): ?><?php $rn=trim((string)($t['invoice_name']??'')); $lbl=$t['name'].($rn!==''&&$rn!==$t['name']?'（'.$rn.'）':''); ?><option value="<?= h($t['id']) ?>" <?= selected($row['talent_id'], $t['id']) ?>><?= h($lbl) ?></option><?php endforeach; ?></select></label><label><span>通貨</span><select name="currency"><option value="USD" <?= selected($row['currency'], 'USD') ?>>USD</option><option value="JPY" <?= selected($row['currency'], 'JPY') ?>>JPY</option></select></label></div>
