...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ((isset($_POST['action']) ? $_POST['action'] : '')) === 'assign_talent') {
    $id = (int)(isset($_POST['id']) ? $_POST['id'] : 0);
    $assignTalentId = trim((string)(isset($_POST['talent_id']) ? $_POST['talent_id'] : ''));
    $assignTalentId = $assignTalentId !== '' ? $assignTalentId : null;

    if ($assignTalentId !== null) {
        $stmt = $pdo->prepare('SELECT COUNT(*) FROM talents WHERE id = ?');
        $stmt->execute([$assignTalentId]);
        if ((int)$stmt->fetchColumn() === 0) {
            set_flash('error', '選択したタレントが見つかりません。');
            redirect_to($baseUrl . '/accounting/journals.php');
        }
    }

    $stmt = $pdo->prepare('SELECT invoice_id FROM accounting_journal_entries WHERE id = ?');
    $stmt->execute([$id]);
    $journal = $stmt->fetch();

    if ($id > 0 && $journal) {
        $pdo->prepare('UPDATE accounting_journal_entries SET talent_id = ?, updated_by = ?, updated_at = NOW() WHERE id = ?')
            ->execute([$assignTalentId, $user['id'] ?: null, $id]);

        if (!empty($journal['invoice_id'])) {
            $pdo->prepare('UPDATE accounting_invoices SET talent_id = ?, updated_by = ?, updated_at = NOW() WHERE id = ?')
                ->execute([$assignTalentId, $user['id'] ?: null, (int)$journal['invoice_id']]);
        }

        write_admin_log($pdo, (int)$user['id'], 'edit', 'accounting_journal', $id, '記帳のタレント紐付けを更新しました');
        set_flash('success', '記帳のタレント紐付けを更新しました。');
    }
          ...
          <?php foreach ($rows as $row): ?>
            <tr>
              ...
              <td class="actions-inline">
                ...
                <?php else: ?>
                  <form method="post" class="actions-inline" style="gap:6px;align-items:center;">
                    <input type="hidden" name="action" value="assign_talent">
                    <input type="hidden" name="id" value="<?= h((string)$row['id']) ?>">
                    <select name="talent_id" style="min-width:140px;">
                      <option value="">タレント未設定</option>
                      <?php foreach ($talents as $t): ?>
                        <option value="<?= h((string)$t['id']) ?>" <?= selected((string)$row['talent_id'], (string)$t['id']) ?>>
                          <?= h($t['name']) ?>
                        </option>
                      <?php endforeach; ?>
                    </select>
