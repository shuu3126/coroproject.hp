...
<main class="page-container">
  ...
  <form method="get" class="search-bar" style="display:flex;gap:8px;align-items:center;margin-bottom:16px;">
    ...
    <?php if ($q !== '' || $filterStatus !== ''): ?>
      <a class="ghost-btn" href="<?= h($baseUrl) ?>/messages.php">リセット</a>
    <?php endif; ?>
  </form>
      ...
      <tbody>
        ...
        <?php foreach ($rows as $row): ?>
          ...
          <tr <?= $row['status'] === 'unread' ? 'style="font-weight:600;"' : '' ?>>
            ...
            <td>
              <a class="ghost-btn" style="font-size:0.8em;padding:4px 10px;" href="<?= h($baseUrl) ?>/message_detail.php?id=<?= (int)$row['id'] ?>">詳細・返信</a>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
