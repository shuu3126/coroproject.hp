...
$id = (int)($_GET['id'] ?? 0);
if (!$id) {
    redirect_to($baseUrl . '/inquiries/messages.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'reply') {
        ...
        if ($body === '') {
            ...
            redirect_to($baseUrl . '/inquiries/message_detail.php?id=' . $id);
        }
  ...
  <div class="card form-card form-stack">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;">
      ...
      <div class="actions-inline">
        <a class="ghost-btn" href="<?= h($baseUrl) ?>/inquiries/messages.php">← 一覧へ戻る</a>
    ...
    <div class="card form-card form-stack">
      ...
      <?php if (empty($settings['smtp_host'])): ?>
        <div class="alert-box alert-error" style="font-size:0.85em;">
          SMTP設定が未設定です。<a href="<?= h($baseUrl) ?>/system/settings.php">設定画面</a>でSMTP情報を入力するとメールが送信されます。返信内容はDBに保存されます。
        </div>
      <?php endif; ?>
