...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    redirect_to($baseUrl . '/talent_portal.php');
}

$accounts = accounting_portal_accounts_list($pdo);
$talents  = accounting_fetch_talents($pdo);
  ...
  <div class="card table-card mt-24">
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          <?php if (!$accounts): ?>
            <tr><td colspan="6" class="empty-state">まだポータルアカウントがありません。</td></tr>
          <?php endif; ?>
          <?php foreach ($accounts as $acc): ?>
            <tr>
              ...
              <td class="actions-inline">
                <button class="ghost-btn" type="button" onclick="showEditModal(<?= (int)$acc['id'] ?>, '<?= h($acc['login_id']) ?>', <?= (int)$acc['is_active'] ?>)">編集</button>
