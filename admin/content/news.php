...
$sql = 'SELECT n.id, n.title, n.date, n.tag, n.thumb, n.excerpt, n.url, n.is_published, n.sort_order, n.talent_id, t.name AS talent_name FROM news n LEFT JOIN talents t ON t.id = n.talent_id';
$params = [];

if ($q !== '') {
    $sql .= ' WHERE n.title LIKE ? OR n.tag LIKE ? OR t.name LIKE ?';
    $params = ["%{$q}%", "%{$q}%", "%{$q}%"];
}
...
<main class="page-container">
  <section class="page-header-block with-actions">
    ...
    <a class="primary-btn" href="<?= h($baseUrl) ?>/content/news_edit.php">新しく追加する</a>
  </section>

  <form method="get" class="card form-card form-grid two">
    <label>
      <span>タイトル・タグ・タレントで検索</span>
      <input type="text" name="q" value="<?= h($q) ?>">
    </label>
    <div class="actions-inline" style="align-self:end;">
      <button class="ghost-btn" type="submit">検索する</button>
      <a class="ghost-btn" href="<?= h($baseUrl) ?>/content/news.php">条件をリセット</a>
    </div>
  </form>
  ...
  <div class="card table-card mt-24">
    <div class="table-wrap">
      <table>
        ...
        <?php if (!$rows): ?>
          <tr>
            <td colspan="8" class="empty-state">まだニュースがありません。</td>
          </tr>
        <?php endif; ?>
        ...
        <?php foreach ($rows as $row): ?>
          <?php
            ...
            $editUrl = $baseUrl . '/content/news_edit.php';
