...
if ($method === 'PATCH' && $id) {
    $body    = api_input();
    $allowed = ['status', 'due_date', 'note', 'payment_bank_info', 'paid_at'];
    $sets    = [];
    $params  = [];
    ...
    $params[] = $id;
    $stmt = $pdo->prepare("UPDATE accounting_invoices SET " . implode(', ', $sets) . " WHERE id = ?");
    $stmt->execute($params);
    if ($stmt->rowCount() === 0) { api_error(404, 'Invoice not found'); }
...
// GET /api/invoices?status=issued&talent_id=x&division=production
if ($method === 'GET') {
    $where  = ['1=1'];
    $params = [];
    ...
    $stmt = $pdo->prepare("
        SELECT i.id, i.invoice_no, i.status, i.division,
               i.close_year, i.close_month, i.subject, i.amount_jpy,
               i.fx_rate, i.due_date, i.paid_at,
        ...
        ORDER BY i.id DESC
    ");
    if (!$stmt) { api_error(500, 'Query prepare failed: ' . implode(' ', $pdo->errorInfo())); }
    $stmt->execute($params);
