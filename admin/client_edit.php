...
if ($isEdit) {
    ...
    $stmt->execute([$id]);
    $found = $stmt->fetch();
    if (!$found) { set_flash('error', 'クライアントが見つかりません。'); redirect_to($baseUrl . '/clients.php'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '') { set_flash('error', '名前は必須です。'); redirect_to($baseUrl . '/client_edit.php' . ($isEdit ? '?id=' . urlencode($id) : '')); }
    ...
    try {
        ...
        redirect_to($baseUrl . '/clients.php');
    } catch (Exception $e) {
        set_flash('error', '保存に失敗しました: ' . $e->getMessage());
        redirect_to($baseUrl . '/client_edit.php' . ($isEdit ? '?id=' . urlencode($id) : ''));
    }
}
...
<main class="page-container narrow">
  ...
  <form method="post" class="card form-card form-stack">
    ...
    <div class="actions-inline">
      ...
      <a class="ghost-btn" href="<?= h($baseUrl) ?>/clients.php">一覧へ戻る</a>
    </div>
  </form>
</main>
<?php end_page(); ?>
