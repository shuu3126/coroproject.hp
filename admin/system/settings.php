...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($action === 'export_admin_data') {
        if (!$canManageUsers) {
            ...
            redirect_to($baseUrl . '/system/settings.php');
        }
    ...
    if ($action === 'update_admin_user') {
        ...
        if ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            set_flash('error', 'メールアドレスの形式が正しくありません。');
            redirect_to($baseUrl . '/system/settings.php?edit_user_id=' . $targetId . '#settings-admin-users');
        }
    ...
    if ($publicXUrl !== '' && !filter_var($publicXUrl, FILTER_VALIDATE_URL)) {
        ...
        redirect_to($baseUrl . '/system/settings.php#settings-public-links');
    }
            ...
            <?php foreach ($adminUsers as $adminUser): ?>
              <tr <?= ($editTargetUser && (int)$editTargetUser['id'] === (int)$adminUser['id']) ? 'style="background:var(--bg);"' : '' ?>>
                ...
                <td>
                  <div class="actions-inline">
                    <a class="ghost-btn"
                       href="<?= h($baseUrl) ?>/system/settings.php?edit_user_id=<?= (int)$adminUser['id'] ?>#settings-admin-users">編集</a>
        ...
        <div class="card form-card form-stack" style="margin-top:16px;border:2px solid var(--primary);">
          ...
          <form method="post" class="form-stack">
            ...
            <div class="actions-inline">
              ...
              <a class="ghost-btn" href="<?= h($baseUrl) ?>/system/settings.php#settings-admin-users">キャンセル</a>
            </div>
          </form>
        </div>
      <?php else: ?>
