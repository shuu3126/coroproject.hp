    ...
    <table class="cp-table">
      ...
      ?>
        <tr>
          ...
          <td data-label="カテゴリ"><?= cp_h($project['category']) ?></td>
          <td data-label="ステータス"><span class="cp-badge <?= cp_h(cp_project_status_class($project['status'])) ?>"><?= cp_h($project['status']) ?></span></td>
          <td data-label="納期" style="<?= $deadlineStyle ?>"><?= cp_h(cp_format_date($project['deadline'] ?? '')) ?></td>
          <td data-label="支払予定" class="cp-text-right"><?= cp_h(cp_format_money($project['creator_amount'] ?? 0)) ?></td>
          <td data-label="更新日"><?= cp_h(cp_format_datetime($project['updated_at'] ?? '')) ?></td>
          <td data-label="操作"><a class="cp-btn" href="<?= cp_h($creativePortalBase) ?>/project.php?id=<?= urlencode($project['id']) ?>">詳細</a></td>
        </tr>
      <?php endforeach; ?>
      </tbody>
    </table>
